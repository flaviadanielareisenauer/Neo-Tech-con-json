...
module.exports = {
  ...
  crearProducto: (req, res) => {
    ...
    let productoNuevo = {
      ...
      image: arrayImages.length > 0 ? arrayImages : ["nuevos/img-default.jpg"],
    };
  ...
  update: (req, res) => {
    let { name, price, discount, category, description, mark, code, color } =
      req.body;
    let arrayImages = [];
    if (req.files) {
      req.files.forEach((image) => {
    ...
    products.forEach((product) => {
      if (product.id === +req.params.id) {
        (product.id = product.id),
          ...
          (product.color = color),
          (product.image =
            arrayImages.length > 0 ? arrayImages : product.image);
      }
    });
